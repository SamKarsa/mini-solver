...
def solve(model: LPModel) -> SolverResult:
    ...
    coefficients = model.objective_coefficient
    if model.sense == 'max':
    ...
    for r in model.restrictions:
        if r.type == '<=':
            A_ub.append(r.coefficient)
            b_ub.append(r.rhs)
        elif r.type == '>=':
            # Multiplicamos por -1 para convertir >= en <=
            A_ub.append([-c for c in r.coefficient])
            b_ub.append(-r.rhs)
        elif r.type == '==':
            A_eq.append(r.coefficient)
            b_eq.append(r.rhs)
    
    #Ejecutar linprog, i mean solver
    result= linprog(
        c=coefficients,
...
def _interpret_result(result, model: LPModel) -> SolverResult:
    ...
    if result.status != 0: 
        # Si no hay solución óptima, retornamos el estado
        return SolverResult(
            status=status,
